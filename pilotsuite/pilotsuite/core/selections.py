...
class SelectionStore:
    ...
    async def patch(self, zone_id: str, revision: int, changes: Any) -> dict[str, Any]:
        self._zone(zone_id)
        if type(revision) is not int or revision < 0:
            raise InvalidSelection("revision must be a nonnegative integer")
        if not isinstance(changes, dict) or not 1 <= len(changes) <= 500:
            raise InvalidSelection("changes must contain 1 to 500 entity decisions")
        ...
        return await asyncio.to_thread(self._patch, zone_id, revision, dict(changes))

    def _patch(self, zone_id: str, revision: int, changes: dict[str, str]) -> dict[str, Any]:
