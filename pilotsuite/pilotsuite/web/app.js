...
const roleKinds = {temperature:['temperature'], humidity:['humidity'], illuminance:['illuminance'], light:['light'], presence:['motion','occupancy','presence','input_boolean'], reference_temperature:['temperature']};
...
function renderRolePreview() {
  const parts = Object.keys(roleKinds).filter(k => k !== 'reference_temperature').map(role => {
    ...
    const labels = {temperature:'Temperatur', humidity:'Feuchte', illuminance:'Helligkeit', presence:'Präsenz / Bewegung / Raumstatus', light:'Licht'};
