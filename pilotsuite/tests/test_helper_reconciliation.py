...
class HelperReconciliationTests(unittest.TestCase):
    def test_exact_id_requires_inspection_and_name_is_conflict(self):
        ...
        self.assertEqual(["inspect","conflict"],[x["state"] for x in r["items"]])
        self.assertFalse(r["execution"]["allowed"])
    def test_missing_registry_id_does_not_authorize_creation(self):
        r=reconcile_helpers({"helper_plan":[{"domain":"timer","key":"pilotsuite_x"}]},[])
        self.assertEqual(1,r["counts"]["unverified"])
